...
RPROMPT='%B%{$fg[red]%} %(?..[%?])%b'
print -P '%F{#ebebeb}[$USER@$HOST]%f %B%F{#87cefa}$(uname -srm)%f%b · %F{#29ff91}$(uptime --pretty)%f'
...
zstyle ':completion:*:descriptions' format ' -- %B%F{#fefef8}%d%f%b --'
zstyle ':completion:*:messages' format ' -- %B%F{#f3dd6d}%d%f%b --'
zstyle ':completion:*:warnings' format ' -- %B%F{#e4002b}no matches found%f%b --'
